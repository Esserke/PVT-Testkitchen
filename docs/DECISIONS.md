...
## 2026-09-06 — Labels, notifications, honest stock takes

- **Shelf photos never silently lower a count.** The reader now flags `partly_hidden` per item and `view: crowded` for a packed shelf; hidden rows arrive unticked with "partly hidden · saw N", and the sheet warns that a crowded shelf gives a floor, not a total. Photos are reliable for the fridge, freezers, drinks fridge, fruit bowl, egg basket and bathroom (marked photo-friendly in `constants.ts`); deep cupboards still need the number confirmed by hand.
- **New locations** from Kevin's list: fruit bowl, egg basket, spice rack, with fruit, eggs and spices moved into them and four items added.
- **QR shelf labels** (`labels` route): pick items by location, preview the sheet on screen, print three to a row on A4. Each code encodes `#/quick/<item id>`, which opens a stripped-down screen with the quick actions. Verified by decoding a rendered label back to its item.
- **Web push** (migration 0006, function `notify`): VAPID keys are generated on first use and kept in `push_config`, readable only by the service role, so no key is ever handled by a person or committed. Subscriptions are per device. A GitHub Actions cron calls `notify` with `action: run` at 15:00 UTC (17:00 local); `notification_log` has a unique key on household, kind and date so nothing is sent twice in a day. Three nudges: town trip tomorrow, Sunday planning when next week is mostly empty, and three or more items low.
- **Service worker** moved from `generateSW` to `injectManifest` (`app/src/sw.ts`) so it can handle `push` and `notificationclick`; clicking a notification focuses an open tab or opens the app at the right screen.
