<script lang="ts">
  ...
  async function onFile(e: Event) {
    ...
    if (!file || !kind) return
    busy = kind === 'shelf_photo' ? 'Reading the shelf' : kind === 'receipt' ? 'Reading the slip' : 'Looking at the plate'
    try {
      const image = await shrinkImage(file)
      const result = await parse(kind, { image, location: kind === 'shelf_photo' ? location : undefined })
...
{#if proposal}
  <ProposalSheet kind={proposal.kind} result={proposal.result} location={proposal.kind === 'shelf_photo' ? location : null} onclose={() => (proposal = null)} />
{/if}

<style>
  .bar { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
