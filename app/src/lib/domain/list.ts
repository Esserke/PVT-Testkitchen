...
export type AutoReason = 'below_par' | 'plan' | 'predicted'
...
export function mergeNeeds(belowPar: { item: Item; quantity: number }[], plan: { item: Item; quantity: number }[], predicted: { item: Item; quantity: number }[] = []): Need[] {
  const m = new Map<string, Need>()
  for (const n of predicted) m.set(n.item.id, { ...n, reason: 'predicted' })
  for (const n of belowPar) {
    const cur = m.get(n.item.id)
    m.set(n.item.id, { item: n.item, quantity: Math.max(n.quantity, cur?.quantity ?? 0), reason: 'below_par' })
  }
  for (const n of plan) {
    const cur = m.get(n.item.id)
...
export function reconcile(lines: ListLine[], needed: Need[]): ListDiff {
  ...
  for (const l of live) {
    if (l.checked || !l.item_id || l.reason === 'manual') continue
  ...
  return {
    ...
    remove: live.filter((l) => l.reason !== 'manual' && !l.checked && l.item_id && !neededById.has(l.item_id)),
    update,
  }
}
