...
import { belowParItems, reconcile, mergeNeeds } from './list'
...
describe('reconcile', () => {
  it('adds missing needs and removes stale automatic lines only', () => {
    ...
    const diff = reconcile(lines, [{ item: item('a'), quantity: 1, reason: 'below_par' }, { item: item('e'), quantity: 2, reason: 'plan' }])
    expect(diff.add.map((n) => n.item.id)).toEqual(['e'])
    expect(diff.remove.map((l) => l.id)).toEqual(['l2'])
    expect(diff.update).toEqual([])
  })
  it('raises quantity and reason when the plan needs more', () => {
    const lines = [line('l1', 'a', 'below_par')]
    const diff = reconcile(lines, [{ item: item('a'), quantity: 3, reason: 'plan' }])
    expect(diff.update.map((l) => [l.id, l.quantity, l.reason])).toEqual([['l1', 3, 'plan']])
  })
  it('merges needs taking the larger quantity', () => {
    const m = mergeNeeds([{ item: item('a'), quantity: 1 }], [{ item: item('a'), quantity: 2 }, { item: item('b'), quantity: 1 }])
    expect(m.map((n) => [n.item.id, n.quantity, n.reason])).toEqual([['a', 2, 'plan'], ['b', 1, 'plan']])
  })
})
