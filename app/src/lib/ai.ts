...
export type Kind = 'message' | 'shelf_photo' | 'receipt' | 'plate' | 'recipe_url'
...
export type Result = MessageResult | ShelfResult | ReceiptResult | RecipeResult
...
export async function parse<T extends Result>(kind: Kind, payload: { text?: string; url?: string; location?: string; image?: { data: string; media_type: string } }): Promise<T> {
