...
export type Kind = 'message' | 'shelf_photo' | 'receipt' | 'plate' | 'recipe_url' | 'lunchbox'
...
export type Result = MessageResult | ShelfResult | ReceiptResult | RecipeResult | LunchboxResult
...
export async function parse<T extends Result>(kind: Kind, payload: { text?: string; url?: string; location?: string; mode?: 'packed' | 'home'; image?: { data: string; media_type: string } }): Promise<T> {
